"""Generate EXP-009A-R2 aggregate features and exact evidence."""
...
def _hash_coefficients(value: Any) -> str:
...
def main() -> None:
    ...
    if manifest["irishman_revision"] != IRISHMAN_REVISION:
        raise ValueError("Manifest revision does not match the frozen R2 contract.")
    ...
    parsed = [extract_coarse_features_r2(record["abc notation"]) for record in selected_records]
        ...
        "records": [
            {
                ...
                "inline_field_counts": result.inline_field_counts,
    ...
    qubo = {
        ...
        **qubo_coefficients,
        "coefficient_sha256": _hash_coefficients(qubo_coefficients),
    }
    ising_coefficients = {
    ...
    ising = {
        ...
        "variable_order": [setting["setting_id"] for setting in settings],
        **ising_coefficients,
        "coefficient_sha256": _hash_coefficients(ising_coefficients),
    }
    feasible_energies = [
        {"bitstring": bitstring, "energy": round(energy, 15)}
        for bitstring, energy, feasible in exact["states"]
        if feasible
    ]
    exact_public = {key: value for key, value in exact.items() if key not in {"states", "model"}}
    exact_public["raw_minimum_energy"] = float(exact["minimum_energy"])
    ...
    exact_public["optimum_class_size"] = len(exact_public["optimum_bitstrings"])
    exact_public["uniform_feasible_subspace_baseline"] = 16 / 256
    ...
    exact_public["feasible_state_energies"] = feasible_energies
    ...
    report = f"""# EXP-009A-R2 exact report

Exact enumeration is authoritative. All 256 assignments were evaluated directly, as a QUBO, and
as an Ising Hamiltonian after the separately frozen closed-metadata parser proof.

- Global optimum bitstrings: {optima}
- Canonical optimum: `{exact_public["canonical_optimum"]}`
- Raw minimum energy: `{exact_public["raw_minimum_energy"]!r}`
- Displayed minimum energy: `{exact_public["displayed_minimum_energy"]}`
...
- Optimum-class size: `{exact_public["optimum_class_size"]}`
- Uniform optimum-class baseline: `{exact_public["uniform_optimum_class_baseline"]:.8f}`
- Uniform feasible-subspace baseline: `{exact_public["uniform_feasible_subspace_baseline"]:.8f}`
...
state costs at least 7. The optimum belongs to this frozen coarse objective and is not an authentic,
correct, or culturally superior selection. No QAOA or hardware result contributes to exact truth.
"""
    (args.output_dir / "EXACT-REPORT.md").write_text(report, encoding="utf-8")
    print("EXP-009A-R2 exact gate passed for 256 assignments; no notation emitted.")
