"""EXP-010D read-only metadata preflight; contains no submission surface."""
...
PLAN = ROOT / "experiments" / "EXP-010D-hardware-parameter-landscape"
OUT = ROOT / "experiments" / "EXP-010D-hardware-parameter-landscape-run"
BACKEND = "ibm_fez"
LAYOUT = [20, 21, 23, 22]
SEED = 44
SHOTS = 4096
PUB_COUNT = 32
USAGE_CEILING_S = 180.0
...
def load_and_validate_rows() -> list[dict[str, Any]]:
    ...
    for block, expected in expected_anchors.items():
        ...
        if tuple(row["kind"] for row in block_rows[:2]) != expected or len(block_rows) != 8:
...
def run(service: Any) -> tuple[str, dict[str, Any]]:
    ...
    # Conservative circuit-time bound plus one second of overhead for every PUB.
